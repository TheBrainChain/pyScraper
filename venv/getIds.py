...
#    print(test.a)
    ...
    DriveResults = DriveService.files().list(q="name = 'SSB_RunLog12'", pageSize=50, fields="nextPageToken, files(id, name)").execute()
    ...
    filename = SpreadsheetService.spreadsheets().values().batchGet(
    ...
    for taskNumber in range(0,6):
        fileToRead = "datafiles"+"/"+subjectInits+tasks[taskNumber]+dateOfExp+endOfLine[1:4]+"/"+subjectInits+tasks[taskNumber]+dateOfExp+endOfLine   #modify directory based on what computer this is on
